...
2. Запустите из корня репозитория `mvn -DskipTests package`.
   - Maven сначала соберёт облегчённый модуль ProtocolLib 5.1.0, поэтому проект компилируется офлайн без коммитов бинарных JAR-ов. Для перехвата DISCONNECT-пакетов на сервере всё равно потребуется настоящая ProtocolLib.
